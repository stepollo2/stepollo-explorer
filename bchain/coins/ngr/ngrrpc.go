...
// EnergiNet type specifies the type of ethereum network
...
func (b *EnergiRPC) getBestHeader() (*ethtypes.Header, error) {
...
// EnergiTypeEstimateGas returns estimation of gas consumption for given transaction parameters
func (b *EnergiRPC) EnergiTypeEstimateGas(params map[string]interface{}) (uint64, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.timeout)
	defer cancel()
...
// EnergiTypeGetBalance returns current balance of an address
func (b *EnergiRPC) EnergiTypeGetBalance(addrDesc bchain.AddressDescriptor) (*big.Int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.timeout)
	defer cancel()
	return b.client.BalanceAt(ctx, ethcommon.BytesToAddress(addrDesc), nil)
}

// EnergiTypeGetNonce returns current balance of an address
func (b *EnergiRPC) EnergiTypeGetNonce(addrDesc bchain.AddressDescriptor) (uint64, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.timeout)
	defer cancel()
