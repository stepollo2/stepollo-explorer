...
// EnergiTypeAddressDescriptorLen - in case of EnergiType, the AddressDescriptor has fixed length
...
// GetChainType returns EnergiType
...
// EnergiTypeGetErc20FromTx returns Erc20 data from bchain.Tx
func (p *EnergiParser) EnergiTypeGetErc20FromTx(tx *bchain.Tx) ([]bchain.Erc20Transfer, error) {
...
// EnergiTxData contains ethereum specific transaction data
type EnergiTxData struct {
...
// GetEnergiTxData returns EnergiTxData from bchain.Tx
func GetEnergiTxData(tx *bchain.Tx) *EnergiTxData {
	etd := EnergiTxData{Status: txStatusPending}
	csd, ok := tx.CoinSpecificData.(completeTransaction)
	if ok {
		if csd.Tx != nil {
